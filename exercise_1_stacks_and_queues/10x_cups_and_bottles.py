...
# given another sequence of integers – each indicating a bottle's capacity (in litters). Your job is to try to fill
# up all the cups.
...
# its integer value by N. When a cup's integer value reaches 0 or less, it gets removed. It is possible that the current
# cup's value is greater than the current bottle's value. In that case, you pick bottles until you reduce the cup's
# integer value to 0 or less. If a bottle's value is greater or equal to the cup's current value, you fill up the cup,
# and the remaining water becomes wasted. You should keep track of the wasted litters of water and print them at the end
# of the program. If you have managed to fill up all the cups, print the remaining water bottles, from the last entered
# – to the first. Otherwise, you must print the remaining cups ordered by the entrance – from the first entered –
# to the last.
# Input
