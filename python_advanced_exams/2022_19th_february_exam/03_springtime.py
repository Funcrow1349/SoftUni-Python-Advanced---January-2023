...
def start_spring(**spring_data):
    types_of_objects = {}
    final_result = ""

    for key, value in spring_data.items():
        if value not in types_of_objects.keys():
            types_of_objects[value] = []
        types_of_objects[value].append(key)

    types_of_objects = dict(sorted(types_of_objects.items(), key=lambda x: (-len(x[1]), x[0])))

    for key, values in types_of_objects.items():
        final_result += f"{key}:\n"
