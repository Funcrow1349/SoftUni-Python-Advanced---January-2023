...
#   As you know, stewards are needed for every single flight. Today you will be that one steward, and you will be
#   assisting the passengers in finding their seats.
...
while len(matches) < 3 and rotations < 10:
    ...
    rotations += 1
    ascii_value = chr(first_number + second_number)
    seats_to_check = {str(first_number) + ascii_value, str(second_number) + ascii_value}
    match_found = seats_to_check.intersection(seats)

    if match_found:
        matched_seat = match_found.pop()
        if matched_seat in matches:
            continue
        matches.append(matched_seat)
    else:
