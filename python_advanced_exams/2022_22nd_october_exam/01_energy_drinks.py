...
#   course, drinks a lot of energy drinks. Stamat's friends are concerned about him and want you to create a program
...
#       both the milligrams of caffeinе and the drink from their sequences. Also, add the caffeine to Stamat's total
#       caffeine.
#   •	If Stamat is about to exceed his maximum caffeine per night, do not add the caffeine to Stamat’s total caffeine.
#       Remove the milligrams of caffeinе and move the drink to the end of the sequence. Also, reduce the current
#       caffeine that Stamat has taken by 30 (Note: Stamat's caffeine cannot go below 0).
...
while energy_drinks and milligrams_of_caffeine:
    current_caffeine = milligrams_of_caffeine[-1] * energy_drinks[0]
    if current_caffeine + caffeine_level <= 300:
        milligrams_of_caffeine.pop()
        energy_drinks.popleft()
        caffeine_level += current_caffeine
    else:
        milligrams_of_caffeine.pop()
...
print(f"Stamat is going to sleep with {caffeine_level} mg caffeine.")