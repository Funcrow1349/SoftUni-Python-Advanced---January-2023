#   It's time for one of the biggest races in the world, Paris-Dakar. The organizers of the event want you to do a
#   program that helps them track the cars through the separate stages in the event.
#
...
#   On the next N lines you will be given the rows of  the matrix (string sequences, separated by whitespace), which
#   will be representing the race route. The tracked race car always starts with coordinates [0, 0]. There will be a
#   tunnel somewhere across the race route. If the race car runs into the tunnel , the car goes through it and exits at
#   the other end. There will be always two positions marked with "T"(tunnel). The finish line will be marked with "F".
...
#       current move. The tunnel stays behind the car, so the race route is clear, and both the positions marked with
#       "T", should be marked with ".".
#   •	If the car reaches the finish line - "F" position, the race is over. The tracked race car manages to finish the
#       stage and the following output should be printed on the Console: "Racing car {racing number} finished the
#       stage!".
...
#   Output
#   •	If the racing car has reached the finish line before the "End" command is given, print on the Console: "Racing
#       car {racing number} finished the stage!"
...
race_route = [[row for row in input().split()] for i in range(race_size)]
car_pos = [0, 0]
total_km_covered = 0
...
while True:
    ...
    row, col = car_pos[0] + directions[command][0], car_pos[1] + directions[command][1]
    car_pos = [row, col]
    ...
    if race_route[row][col] == "F":
        ...
        for row in range(len(race_route)):
            if "T" in race_route[row]:
                row, col = row, race_route[row].index("T")
                car_pos = [row, col]
                race_route[row][col] = "."
    else:
        total_km_covered += 10

print(f"Distance covered {total_km_covered} km.")
