...
#   A string represents the snake. It starts moving from the top-left corner to the right. When the snake reaches the
#   end of the row, it slithers its way down to the next row and turns left. The moves are repeated to the very end.
...
phrase_copy = deque(phrase)

for row in range(rows):
    while len(phrase_copy) < columns:
        phrase_copy.extend(phrase)

    if row % 2 == 0:
        print(*[phrase_copy.popleft() for _ in range(columns)], sep="")
    else:
        print(*[phrase_copy.popleft() for _ in range(columns)][::-1], sep="")